...
import io.pragra.capstonestudentregistration.entity.*;
...
public class CapstoneStudentRegistrationApplication {
    ...
    CommandLineRunner runner(){
        return args -> {
            ...
            Education education = new Education("Bachelors","Information Technology","Gujarat University",2015,1);
            //education.getStudent().setStudentId(student.getStudentId());

            Employer employer = new Employer("TCS","Tata Consultancy Services",1243543534,1,false);
            Payment payment =new Payment(50,30,3500,2,1);

            service.addRegistration(new Registration(student, Arrays.asList(program1),Arrays.asList(education),Arrays.asList(employer),payment,new Date()));
        };
    }
}
